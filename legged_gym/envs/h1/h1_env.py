...
#         # 2. gravity (3:6)
#         noise_vec[3:6] = noise_scales.gravity * noise_level
#         # 3. commands (6:9) - No noise
#         noise_vec[6:9] = 0.
#         # 4. dof_pos (9:28)
#         noise_vec[9:9+self.num_actions] = noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
#         # 5. dof_vel (28:47)
#         noise_vec[9+self.num_actions:9+2*self.num_actions] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
#         # 6. actions (47:66) - No noise (previous actions)
#         noise_vec[9+2*self.num_actions:9+3*self.num_actions] = 0.
#         # 7. diff_pos (66:85) - No noise for error term usually
#         noise_vec[9+3*self.num_actions:9+4*self.num_actions] = 0.
#         # 8. sin/cos (85:87) - No noise
#         noise_vec[9+4*self.num_actions:9+4*self.num_actions+2] = 0.
...
#         max_phase_steps = max(int(period / self.dt), 1)
#         rand_steps = torch.randint(
#             low=0, high=max_phase_steps, size=(len(env_ids),),
...
#             self.dof_vel * self.obs_scales.dof_vel,
#             self.actions,
#             diff_pos,   # <--- Critical for IL
#             sin_phase,
#             cos_phase
#         ), dim=-1)
        
#         # Copy to privileged (for now, same as obs)
...
#     def _reward_tracking_joint_pos(self):
#         """Imitation reward: track reference joint positions."""
#         ref_pos, _ = self._get_ref_state()
#         error = torch.sum(torch.square(self.dof_pos - ref_pos), dim=1)
#         sigma = float(getattr(self.cfg.rewards, "tracking_sigma", 0.25))
#         return torch.exp(-error / sigma)
...
#     # [Deleted] _reward_hip_pos: 
#     # Removed to avoid index errors with H1 robot. 
#     # Since weight is 0.0 in config, this function is not needed.
